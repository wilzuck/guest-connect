...
export function Footer() {
  ...
  if (/(^|\/)(login|signup)$/.test(pathname)) return null;
   if (/(^|\/)service-management(\/|$)/.test(pathname)) return null;
  ...
  const columns = [
    ...
    },
    {
      ...
      links: [
        ...
        { label: t("siteMap"), href: `/${locale}/plan-du-site` },
      ],
    },
  ];
      ...
      <Container className="py-14 sm:py-16">
        <div className="grid gap-12 lg:grid-cols-12">
          <div className="lg:col-span-4">
            ...
            <p className="mt-5 max-w-sm text-[15px] leading-7 text-zinc-600 dark:text-zinc-400 sm:text-sm">
              {t("tagline")}
            </p>
          ...
          <div className="grid gap-10 sm:grid-cols-3 lg:col-span-8">
            {columns.map((c) => (
              <div key={c.title}>
                <p className="text-base font-semibold dark:text-white sm:text-sm">
                  {c.title}
                </p>
                ...
                <ul className="mt-5 space-y-4 sm:space-y-3">
                  {c.links.map((l) => (
                    <li key={l.href}>
                      <Link
                        href={l.href}
                        className="block py-1 text-[15px] text-zinc-600 transition-colors hover:text-black dark:text-zinc-400 dark:hover:text-white sm:text-sm"
                      >
                        {l.label}
                      </Link>
                    </li>
        ...
        <div className="mt-12 flex flex-col gap-4 border-t border-black/5 pt-7 text-sm text-zinc-500 dark:border-zinc-800 dark:text-zinc-400 sm:flex-row sm:items-center sm:justify-between">
