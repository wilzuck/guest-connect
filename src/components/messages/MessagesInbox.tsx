...
export function MessagesInbox({
  ...
  if (state === "loading") {
    return (
      <section
        className={cn(
          "grid h-[calc(100dvh-4rem)] min-h-[680px] place-items-center border-b border-[#E8E8EC] bg-white",
      ...
      >
        <div className="text-center">
          ...
          <p className="mt-4 text-sm font-semibold text-[#202024]">Chargement des conversations...</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section
      className={cn(
        "h-[calc(100dvh-4rem)] min-h-[680px] overflow-hidden border-b border-[#E8E8EC] bg-white",
        withTopBorder && "border-t",
        !withBottomBorder && "border-b-0",
      )}
    >
      <div className="grid h-full lg:grid-cols-[248px_360px_minmax(0,1fr)]">
        <aside className="hidden border-r border-[#E8E8EC] bg-[#F7F7F8] lg:flex lg:flex-col">
          <div className="border-b border-[#E8E8EC] p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#8E8E93]">
              {mode === "admin" ? "Centre support" : "Espace"}
            </p>
            <h1 className="mt-2 text-lg font-semibold text-[#202024]">{title}</h1>
            {description ? <p className="mt-1 text-xs leading-5 text-[#8E8E93]">{description}</p> : null}
          </div>
          ...
          <div className="mt-auto border-t border-[#E8E8EC] p-4">
            <div className="rounded-2xl border border-[#E8E8EC] bg-white p-3">
              <p className="text-xs font-semibold text-[#202024]">
        ...
        <section
          className={cn(
            "flex min-w-0 flex-col border-r border-[#E8E8EC] bg-white",
            mobilePanel === "chat" && "hidden lg:flex",
          )}
        >
          <div className="border-b border-[#E8E8EC] p-3">
            <label className="flex h-10 items-center gap-2 rounded-lg border border-[#E8E8EC] bg-white px-3 text-sm text-[#8E8E93] shadow-xs shadow-black/5">
          ...
          <div className="flex items-center gap-2 border-b border-[#E8E8EC] px-3 py-2 text-xs font-medium text-[#73737A]">
            <button className="rounded-full bg-[#EAF8FF] px-3 py-1.5 text-[#0789C8]">Messages</button>
            ...
            {filtered.length > 0 ? (
              filtered.map((conversation) => (
                <button
                  ...
                  className={cn(
                    "flex w-full gap-3 border-b border-[#EFEFF2] px-4 py-3 text-left transition hover:bg-[#F7F7F8]",
                    selected?.id === conversation.id && "bg-[#EAF8FF]",
                  )}
                >
                  <Avatar name={conversation.participantName} group={conversation.group} />
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center gap-2">
                      <span className="truncate text-sm font-semibold text-[#202024]">
                        {conversation.participantName}
                      </span>
                      {conversation.group ? <Users className="h-3.5 w-3.5 text-[#0789C8]" aria-hidden="true" /> : null}
                      <span className="ml-auto shrink-0 text-xs text-[#8E8E93]">{conversation.updatedAt}</span>
                    </span>
                    <span className="mt-1 block truncate text-xs font-medium text-[#0789C8]">{conversation.title}</span>
                    <span className="mt-1 block truncate text-xs text-[#73737A]">{conversation.lastMessage}</span>
                  </span>
                  {conversation.unreadCount ? (
                    <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[#EF4444] text-[11px] font-bold text-white">
                      {conversation.unreadCount}
                    </span>
                  ) : null}
                </button>
        ...
        >
          {selected ? (
            <>
              <header className="flex h-16 items-center gap-3 border-b border-[#E8E8EC] px-4">
                <button
                  type="button"
                  className="rounded-lg border border-[#E8E8EC] px-3 py-2 text-xs font-semibold text-[#73737A] lg:hidden"
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-[#202024]">{selected.title}</p>
              ...
              <div className="flex items-center gap-1 border-b border-[#E8E8EC] px-4 py-2">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto bg-[#FAFAFB] px-4 py-5">
              ...
              <footer className="border-t border-[#E8E8EC] bg-white px-3 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
                {replyTo ? (
                  <div className="mb-2 flex items-center gap-2 rounded-xl bg-[#F7F7F8] px-3 py-2 text-xs text-[#73737A]">
                    <Reply className="h-3.5 w-3.5" aria-hidden="true" />
                    <span className="min-w-0 flex-1 truncate">Réponse à : {replyTo.body}</span>
                    <button type="button" className="font-semibold text-[#202024]" onClick={() => setReplyTo(null)}>
                      Annuler
                    </button>
                  </div>
                ) : null}
                ...
                {attachments.length > 0 ? (
                  <div className="mb-2 flex flex-wrap gap-2">
                    {attachments.map((file) => (
                      <span
                        key={file.id}
                        className="inline-flex items-center gap-2 rounded-full bg-[#F7F7F8] px-3 py-1.5 text-xs font-medium text-[#5F5F66]"
                      >
                ...
                <div className="flex items-end gap-2 rounded-2xl border border-[#E8E8EC] bg-white p-2 shadow-xs shadow-black/5">
                  <input
                    ref={fileInputRef}
                    type="file"
                    multiple
                  ...
                  <button
                    type="button"
                    className="grid h-10 w-10 shrink-0 place-items-center rounded-xl text-[#73737A] hover:bg-[#F7F7F8]"
                  ...
                  <textarea
                    ...
                    className="max-h-40 min-h-10 flex-1 resize-none overflow-y-auto bg-transparent px-1 py-2 text-sm outline-none placeholder:text-[#B1B1B7] disabled:cursor-not-allowed"
                  />
                  <button
                    type="button"
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "bg-white text-[#202024] shadow-sm shadow-black/[0.04]" : "text-[#73737A] hover:bg-white",
      )}
    >
      <Icon className="h-4 w-4" aria-hidden="true" />
      <span className="min-w-0 flex-1 truncate">{label}</span>
      {count ? (
        <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[#EF4444] px-1.5 text-[11px] font-bold text-white">
          {count}
        </span>
      ) : null}
    </button>
...
function ToolbarButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "inline-flex h-9 items-center gap-2 rounded-lg border border-transparent px-2.5 text-xs font-semibold transition hover:border-[#E8E8EC] hover:bg-[#F7F7F8]",
        tone === "danger" ? "text-[#E04F5F]" : "text-[#5F5F66]",
      )}
      title={label}
    >
      <Icon className="h-4 w-4" aria-hidden="true" />
    ...
    <article className={cn("flex gap-3", message.mine && "justify-end")}>
      ...
      <div className={cn("max-w-[min(720px,86%)]", message.mine && "text-right")}>
        <div className="mb-1 flex items-center gap-2 text-xs text-[#8E8E93]">
          <span className="font-semibold text-[#5F5F66]">{message.author}</span>
        ...
        <div
          className={cn(
            ...
            message.mine ? "bg-[#05A6D6] text-white" : "bg-white text-[#202024]",
          )}
        >
          {message.replyTo ? (
            <p className={cn("mb-2 border-l-2 pl-2 text-xs", message.mine ? "border-white/50 text-white/75" : "border-[#05A6D6] text-[#73737A]")}>
              {message.replyTo}
            </p>
          ) : null}
          <p>{message.body}</p>
              ...
              {message.attachments.map((file) => (
                <span
                  ...
                  className={cn(
                    ...
                    message.mine ? "bg-white/15 text-white" : "bg-[#F7F7F8] text-[#5F5F66]",
                  )}
                >
        ...
        <button
          type="button"
          onClick={onReply}
          className="mt-1 text-xs font-semibold text-[#8E8E93] hover:text-[#202024]"
        >
          Répondre
        </button>
      </div>
  ...
  return (
    <span
      className={cn(
        ...
        group ? "bg-[#EAF8FF] text-[#0789C8]" : "bg-[#F2F2F3] text-[#5F5F66]",
      )}
    >
...
function EmptyState({ label }: { label: string }) {
  return (
    <div className="grid h-full min-h-64 place-items-center p-8 text-center">
      <div>
        <div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-[#F7F7F8] text-[#8E8E93]">
          <Mail className="h-5 w-5" aria-hidden="true" />
        </div>
        <p className="mt-3 text-sm font-semibold text-[#202024]">{label}</p>
